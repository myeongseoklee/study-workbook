...
src/
├── shared/llm.ts          # 공용 LLM 클라이언트(.env 자동 로드)
├── week0-agent-loop/      # 02장: 프레임워크 없이 에이전트 (돌아가는 기준선)
...
각 소스 파일 하단의 `🛠 직접 해볼 것` 주석이 그 주차의 심화 과제다. 스캐폴드는 "돌아가는 기준선"이니, 먼저 실행해 흐름을 본 뒤 직접 고쳐보라.
