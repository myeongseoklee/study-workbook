...
| `coding-agent-architecture` | 코딩 에이전트 만들기 — 에이전트 루프·도구·컨텍스트(1강), 세션·컴팩션·기업 납품(2강), 관리 서버 구현(3강), 이벤트 기반 에이전트 서버(4강), 브로커 인프라와 로컬 모델 선택(5강). 회차가 늘어나는 시리즈라 `docs/`가 회차 폴더(`ep01-`~`ep05-`) + 누적 문서 구조다 |
...
| `redis-pubsub-realtime` | Redis Pub/Sub로 실시간 알림 만들기 — FastAPI 실습 강의(28분)를 재서술했다. 구독 커넥션이 성격을 바꾸는 지점과 **커넥션 분리 처방의 진짜 근거**(RESP2 제약은 RESP3에서 사라지지만 처방은 남는다), 강사 자신이 안티패턴이라 밝힌 접속별 구독을 팬아웃 레지스트리로 고치는 길, 그리고 강의가 다루지 않은 **운영과 대안 판정**을 공식 문서로 보강했다 — 느린 구독자의 출력 버퍼 강제 종료, 클러스터 전파 비용과 Sharded Pub/Sub, Streams와의 갈림길. 화면 캡처는 한 장도 쓰지 않고 `[MM:SS]` 좌표로 대신했다. 과제 4개가 라우팅·팬아웃·강제 종료 판정·수단 선택을 코드로 옮긴다 |
| `ai-work-delegation` | AI에게 일을 시키는 방식의 재설계 — 답을 받는 대신 후보를 생성시킨다. 연역·귀납·귀추의 분업, 탐색 공간 설계, 전제 부정(케이블 끊기), 역할 분리와 편집자의 자리, 그리고 생산성 지표의 재정의. 소셜 미디어 연재 20항목을 재서술하면서 원문이 출처 없이 인용한 수치 셋을 원 논문까지 확인해 붙였고, 근거 논문이 논쟁 중인 입장문이라는 사실과 반론을 함께 실었다. 과제 4개가 판정 규칙을 실행 가능한 코드로 옮긴다 |
