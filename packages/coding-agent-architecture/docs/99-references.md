...
- **뉴런데브 — 코딩에이전트 만들기 재생목록**: <https://www.youtube.com/playlist?list=PLGjmLnACrYZI>
  - **#6 코딩에이전트 만들기(끝)**: <https://www.youtube.com/live/vMXq7J0zeU0> — 이벤트 소싱 워커 아키텍처, 프리픽스/KV 캐시 진단, AI 협업 원칙. 2026-09-01 스트리밍, 멤버십 전용. 자동 자막 기반 `full-subtitle`이며 수치·고유명사는 화면 캡처로 교차 확인했다.
  - #1 (본편, 약 1시간 54분) · #1-사고로 꺼짐(도입부, 약 23분). 채널: 뉴런데브. 업로드 2026-07-29.
  - **#2 비지니스용 에이전트** (라이브, 약 1시간 23분). 2026-08-04 스트리밍. 멤버십(코친멤버) 전용.
    #4와 마찬가지로 **나중에 `ko-orig` 자동 자막이 생성돼 이중 전사(`full-dual`)가 됐다.** 두 전사를 대조해 이 회차의 핵심 수치 일곱(기반 컨텍스트 5% · 코덱스 약 4% · 컴팩트 80% 트리거 · 유휴 캐시 20분 · 262K · 95%→5% · AGENTS.md 200K 축소)이 **모두 같은 값으로 나오는 것을 확인**했고, STT 환각·중복 의심 구간은 0건이었다. 반면 STT가 뭉갠 구간이 셋 있어 그 내용을 보강했다 — 그중 하나가 **기반 컨텍스트 항목 목록의 출처가 OpenAI 공식 문서라는 사실**이다(3장).
  ...
  - **#5 이벤트 브로커 인프라** (라이브, 약 1시간 11분): <https://www.youtube.com/live/_0HVAaA4x1g> — 4강 골격이 실제로 도는 인프라가 되는 회차 + 로컬 모델 선택 실전. 2026-08-25 스트리밍. **멤버십 전용.**
    5강 자료는 슬라이드 다섯 장을 1차 자료로 쓴다(`핵심은 비동기` · `참가자 구성` · `워크플로우` · `MQ신뢰보증` · `큐로 읽기와 로그로 읽기`). 그중 **다이어그램 세 장만 자료에 실었고**, 불릿 목록 슬라이드 둘은 본문이 표로 다시 썼으므로 좌표만 남겼다.
  - #1·#3은 자동 자막, #2는 자막이 없어 **로컬 STT(whisper large-v3-turbo)로 전사**한 뒤 슬라이드 캡처와 교차 확인해 재구성했다.
  - **#4는 전사문이 둘이다.** 라이브 직후에는 자막이 없어 로컬 STT로 전사했고(`transcript.md`), 며칠 뒤 유튜브 `ko-orig` 자동 자막이 생성돼 그것도 받아 두었다(`transcript.auto.md`). **두 전사를 기계 대조해 자료의 인용문 142건을 전부 검증했다** — 서로 다른 ASR이 서로 다른 방식으로 틀리기 때문에, 두 쪽이 일치하는 지점은 신뢰도가 크게 올라가고 갈리는 지점은 캡처로 판정할 대상이 된다.
  ...
  - **ASR의 한계 (#4에서 실제로 나타난 것)**: 약어와 영문 용어가 흔들린다 — `CPS`→`GPS`/`CTS`, `멱등`→`몇 등`/`몇덩`/`역등`, `선입선출`→`선임 선출`, `어보트`→`어벌트`/`어볼트`. 두 전사가 서로를 교정해 준 예: 로컬 STT의 `엠벨롭`이 자동 자막에서는 `엔벨롭`으로 나와 **엔벨로프**가 확정됐고, 자동 자막의 `SQL 라이트`·`HP`·`속회소`는 STT의 `SQLite`·`HTTP`·`소켓`이 맞다. **#4의 모든 식별자·필드명·타입명은 슬라이드·코드 캡처로 교차 확인한 값**이다.
  - **두 전사가 함께 틀린 것은 근거로 쓰지 않았다.** `UID 7`(문맥상 UUIDv7이 거의 확실하나 화면에 글자가 없었다), 노드 버전(`LTS 23인가 22부터` — 강의자 본인이 흔들었다), 즉흥 나열된 라이브러리 이름. 각각 해당 장에 표기 주의를 달았다.
  - **자동 자막이 STT의 소실 구간을 복원했다.** 로컬 STT는 반복 루프로 `52:30`·`1:29:00`·`48:30` 세 앵커를 뭉갰는데, 그중 `52:30`에는 4장에 필요한 문장이 있었다(*"일반적인 시스템보다 훨씬 더 쉽게 일어나… 키 발급을 안 할 수가 없어요"*). 역방향 검사(STT가 자동보다 크게 긴 구간 = 환각·중복 의심)에서는 **0건**이 나왔다.
...
- **NVIDIA DGX Spark (128GB LPDDR5x 통합 메모리, 273 GB/s, GB10 + 20코어 Arm)**: <https://www.nvidia.com/en-us/products/workstations/dgx-spark/>
  - 5강 12장의 "기업 제안 기준선" 근거. 벤더는 추론 검증 기준 최대 200B급, 두 대 연결 시 405B급까지로 안내한다. **"올라간다"는 벤더 기준이고 "코딩에 쓸 만하다"는 다른 선**이라는 것이 12장의 요지다.
...
- **PGMQ**: <https://github.com/pgmq/pgmq>
  - 3장 베이스. 5강 슬라이드가 든 5연산(`write`·`read`·`delete`·`set_vt`·`archive`)이 실제 함수로 어떻게 대응하는지 확인하라. **`read`가 조회가 아니라 가시성 타임아웃을 설정하는 갱신**이라는 사실이 4장의 "읽기가 쓰기다"의 근거다 — 함수 시그니처에서 직접 확인된다.
- **PostgreSQL — 트랜잭션 격리와 읽기 일관성**: <https://www.postgresql.org/docs/current/transaction-iso.html>
  - 3장의 "읽기 락을 왜 푸는가". PostgreSQL은 MVCC라 일반 `SELECT`가 읽기 락을 걸지 않는다는 점부터 확인하면, 강의가 말한 "락을 풀어놨다"가 무엇을 가리키는지 정확해진다.
- **PostgreSQL — 공유 버퍼**: <https://www.postgresql.org/docs/current/runtime-config-resource.html#GUC-SHARED-BUFFERS>
  - 4장의 "모두 같은 꼬리를 읽으므로 그 페이지는 이미 shared buffer에 있다"의 근거. 커서 tail이 왜 소비자 수에 거의 비례하지 않는지가 여기서 나온다.
- **LM Studio — Speculative Decoding**: <https://lmstudio.ai/docs/app/advanced/speculative-decoding>
  - 11장. 공식 문서는 개념과 초안 모델 선택까지만 다루고 **초안 길이·채택 임계의 필드 이름은 버전에 따라 바뀐다.** 본문이 필드 이름 대신 규칙("길이는 짧게, 임계는 함부로 낮추지 않는다")을 외우게 한 이유다.
- **Gemma 4 QAT (Google)**: <https://blog.google/innovation-and-ai/technology/developers-tools/quantization-aware-training-gemma-4/>
  - 11장의 QAT/PTQ 근거. QAT 체크포인트가 **Q4_0 같은 특정 형식으로 배포된다**는 점(= 비트 선택지가 좁다)과, QAT가 표준 PTQ 기준선보다 전반적 품질이 높다는 제조사 서술을 직접 확인할 수 있다.
- **Node.js `child_process.spawn`**: <https://nodejs.org/api/child_process.html#child_processspawncommand-args-options>
  - 9장의 bash 도구 구현. 표준 입출력이 스트림으로 오고 **부모 → 자식은 별도 경로(`stdin`)라 출력만으로는 단방향**이라는 점이 10장의 "실행 중 개입은 별도 장치가 필요하다"와 이어진다.

> ⚠️ #5 검증 안내: 5강 원본은 **자막이 없는 라이브 아카이브**라 로컬 STT(whisper large-v3-turbo)로 전사했고, **STT 반복 루프가 총 약 20분을 삼켜 다섯 구간을 조건화를 끄고 재전사해 복원**했다(가장 큰 덩어리가 45:16–58:01). 그 결과 앵커 109개·5,164단어에서 **143개·6,817단어**가 됐다. 고유명사는 자동 전사가 특히 자주 틀린다 — `PGMQ`→`PG&Q`/`피지큐`, `Qwen`→`QN`/`Q1`, `Gemma`→`젠마`. **본문의 식별자와 수치는 슬라이드·화면 캡처로 교차 확인한 것만 남겼다.**
>
> 후반부(11·12장)에서 강의자가 즉흥으로 나열한 **개별 모델명과 버전 번호는 STT로도 화면으로도 확정되지 않은 것이 섞여 있어 본문에 옮기지 않았다.** 대신 판단 기준(Q5 하한 · 멀티모달은 별도 모듈 · 용량 공백 · KV 캐시 양자화의 함정)만 남겼다 — 모델 카탈로그는 몇 달이면 바뀌지만 판단은 남기 때문이다. **용량 공백의 "37GB → 82GB"는 2026-08 시점의 화면 관측값**이므로 자기 시점에 다시 확인하라.
...
> ⚠️ #2 검증 안내: #2 본문의 수치 중 **기반 컨텍스트 5%(코덱스 약 4%), 컴팩션 80% 트리거, 유휴 캐시 약 20분, AGENTS.md 200K 축소, 262K 컨텍스트 예시**는 강의자가 라이브에서 제시한 관찰·경험값이며 공식 문서로 확정된 수치가 아니다. **이중 전사 대조로 확인된 것은 "강의자가 그 값을 말했다"까지이고, 그 값이 맞다는 뜻이 아니다** — 특히 `4%`는 강의자 본인이 *"코덱스가 기본으로 한 4% 먹고 시작하나? 그럴 거야"*라고 추측형으로 말했다. 반면 **기반 컨텍스트를 이루는 항목 목록과 그 순서는 화면에 띄운 OpenAI 공식 문서를 따라 읽은 것**이므로 증거 등급이 다르다(3장에 명시). **원리(기반은 작게, 잘림 전에 컴팩트, 캐시는 내려간다)는 취하되 구체 수치는 자기 환경에서 측정하라.** 제품명·모델명 표기는 로컬 STT 전사라 오차가 있을 수 있어 슬라이드 캡처로 확인된 것만 본문에 남겼다.
