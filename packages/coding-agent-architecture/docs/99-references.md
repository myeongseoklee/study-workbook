...
- **뉴런데브 — 코딩에이전트 만들기 재생목록**: <https://www.youtube.com/playlist?list=PLGjmLnACrYZI>
  ...
  - **#3** (라이브, 약 58분): <https://www.youtube.com/live/xBbmtCHgnBA> — 2강에서 설계한 관리 서버의 **구현 시연**. 2026-08-11 스트리밍, 공개.
  - #1·#3은 자동 자막, #2는 **자막이 없어 로컬 STT(whisper large-v3-turbo)로 전사**한 뒤 슬라이드 캡처와 교차 확인해 재구성했다. 본문의 `[00:00:00]` 타임스탬프는 원본 영상에서 직접 확인할 수 있는 좌표다.
  - **자동 자막의 한계**: 고유명사가 흔들린다. #3에서 확인된 예 — `SQ라이트`→SQLite, `도코`→Docker, `민피/탑피/탑케`→min_p/top_p/top_k, `카피베리`→capybara, `잼마`→gemma. 그래서 3강 문서의 식별자·수치는 **화면 캡처로 교차 확인**한 값을 썼다.
