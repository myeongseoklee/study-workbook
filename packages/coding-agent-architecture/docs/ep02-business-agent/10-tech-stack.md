...
> **PGVector에 카프카를 대신할 이벤트 스트림 플러그인도 꽂는다.** 이것을 베이스로 해서 돌린다.
...
> **Express 서버가 웹소켓 서버도 되고 일반적인 웹 백엔드도 되니, 하나만 구동하면 다 된다.**

### 서버 렌더링을 쓰지 않는다

> **"서버 렌더링을 안 쓸 거예요. 문제를 많이 일으켜서."**

대신 이렇게 한다.

> 프론트엔드는 **별도로 빌드**되어 `dist`가 나온다. 그 `dist`를 **백엔드 서버가 정적으로 서빙**한다. 프론트엔드 서버와 백엔드 서버를 따로 만들지 않고 **서버 한 대에서 다 한다.**
...
> **`apps`만 `packages`를 참조해야 한다.** `packages`가 핵심 도메인이므로 오케스트레이션을 참조할 리가 없다. 단방향으로 유지한다.
>
> 그래서 `packages`에는 **react나 express 같은 지식과 무관한 순수 도메인 로직과 모델만** 들어간다. 실제로 `packages`의 의존성을 열어보면 최소한만 있다.
...
> 라이브러리는 많이 쓰지 않는다. **"기술 스택이 복잡해봐야 소용없으니 이 정도에서 정리한다."**
