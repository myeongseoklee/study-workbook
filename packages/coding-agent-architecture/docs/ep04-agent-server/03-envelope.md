...
> 이벤트 발급도 특정 리더가 발급할 게 아닌 이상은 UID 같은 걸로 발급할 수밖에 없어. **유니파이하게 식별이 되어야 되기 때문에** 대부분 이벤트 키는 **UID 7**이나 이런 걸로 발급하거든요. `[19:00]`

> **표기 주의:** 이 대목은 로컬 STT와 유튜브 자동 자막이 **둘 다 `UID 7`**로 옮겼다. 즉 화면에 글자로 나오지 않았고 음성만 있는 구간이다. 문맥상 **UUIDv7**을 말한 것이 거의 확실하지만(전역 유일 + 시간 정렬 요구), 캡처로 확인되지 않았으므로 자료는 인용을 들린 대로 두고 해석만 아래에 붙인다. "유니파이하게"도 두 전사가 일치하며 문맥상 "유니크하게"의 발음 흔들림으로 보인다.
