...
> **이름이 두 벌인 것에 주의.** 슬라이드는 같은 화면에서 **"꺼내, 지워, 대기"**(실제 큐 동작)와 **"택배 보내기/조회/취소"**(접수처 비유)를 나란히 쓰고, 위 인용은 비유 쪽 어휘를 API 이름처럼 말한 것이다. **실제 PGMQ 함수는 `send` / `read(queue, vt, qty)` / `set_vt` / `delete` / `archive`** 다섯이다. 셋으로 세는 것은 **서버가 알아야 하는 동작이 셋**이라는 뜻이고, 그 셋은 「보내기·조회·취소」가 아니라 **「꺼내기·지우기·점유 연장」**이다 — `send`는 발행하는 쪽 몫이고 취소에 해당하는 동작은 없다. 이 구분이 중요한 이유는 아래 「삭제 시점」이 `delete`와 `set_vt`의 구분 위에 서 있기 때문이다.
