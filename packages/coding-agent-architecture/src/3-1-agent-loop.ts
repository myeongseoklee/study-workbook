 ...
 *
 * 성공 기준 (테스트가 검사하는 항목)
 *  - 도구 결과가 다음 모델 호출의 입력으로 들어간다
 *  - 모델이 done을 반환하면 그 시점에 멈추고 최종 응답을 돌려준다
 *  - maxIterations에 도달하면 멈춘다 (무한 루프 불가)
 *  - 도구가 던진 에러를 삼키지 않는다 — 에러 메시지를 관찰 결과로 모델에게 돌려준다
 *  - 이터레이션 기록(trace)에 각 호출의 도구명과 결과가 순서대로 남는다
 */
