...
파트 1은 [`../docs/90-must-memorize.md`](../docs/90-must-memorize.md)의 항목 **66개와 1:1 대응**한다. 회차가 쌓이면 카드도 쌓이므로 권장 상한(20문항)을 넘는다. 대신 **강의별·주제별 13묶음(A~M)** 으로 나눴으니 한 번에 한 묶음씩 풀면 된다. 1강만 들었다면 A~D까지만 풀면 된다.

| 묶음 | 범위 | 문항 |
|---|---|---|
| A~D | **1강** — 에이전트 개념·루프·도구·훅·컨텍스트·로컬 LLM | 21 |
| E~H | **2강** — 세션·컨텍스트 조립·컴팩션·비즈니스 요구 | 25 |
| I~M | **3강** — 저장소·계정·승인·세션 감사·구조 가드레일·모델 레지스트리·조직 트리 | 20 |
...
**1-K-4.** i18n 리소스 키를 타입으로 선언하는 이유를 쓰라. 문자열로 쓰면 오타가 언제 드러나는가?

### 묶음 L — 3강: 모델과 조직 · 4문항

**1-L-1.** 프리셋이란 무엇인가? 왜 등록 단위가 "모델"이 아닌지 예를 들어 설명하라.

**1-L-2.** 조직 루트를 하나로 만드는 것이 왜 흔한 실수인가? 복수 허용이 기본값이어야 하는 이유를 "비대칭"으로 설명하라.

**1-L-3.** 조직이 존재하는 큰 이유 하나를 대라. 할당의 기본값은 무엇이며 할당 단위는 무엇인가?

**1-L-4.** 관리자 권한 범위 두 종류를 쓰고, 하위 계층 전체 범위가 없으면 무엇이 안 되는지 설명하라. 위임의 제약은?
...
**1-M-3.** "AI가 코드를 쓸 때 단위 테스트는 옛날 방식"이라는 주장의 근거를 쓰라. 그리고 이 주장이 **성립하지 않는 영역**을 예와 함께 대라.
...
- **"단위 테스트는 이제 필요 없다"로 정리했는가?** → 3강 08장 + 1-M-3. 전제가 붙은 주장이다 — 순수 로직에는 여전히 명세로 쓴다.
