...
파트 1은 [`../docs/90-must-memorize.md`](../docs/90-must-memorize.md)의 항목 **69개와 1:1 대응**한다. 회차가 쌓이면 카드도 쌓이므로 권장 상한(20문항)을 넘는다. 대신 **강의별·주제별 13묶음(A~M)** 으로 나눴으니 한 번에 한 묶음씩 풀면 된다. 1강만 들었다면 A~D까지만 풀면 된다.

| 묶음 | 범위 | 문항 |
|---|---|---|
| A~D | **1강** — 에이전트 개념·루프·도구·훅·컨텍스트·로컬 LLM | 21 |
| E~H | **2강** — 세션·컨텍스트 조립·컴팩션·비즈니스 요구 | 25 |
| I~M | **3강** — 저장소·계정·승인·세션 감사·구조 가드레일·모델 레지스트리·조직 트리 | 23 |
...
### 묶음 I — 3강: 저장소·계정·승인 · 6문항

**1-I-1.** 사내 비즈니스 서버의 기본 저장소는 무엇이고, 그 선택의 판단 기준은 무엇인가? 파일시스템+JSON을 쓰지 않는 이유 두 가지도 대라.

**1-I-2.** 회사 이메일을 계정의 영구 키로 쓰면 안 되는 이유를 쓰라. 무엇이 무너지는가?

**1-I-3.** 마스터 계정을 DB가 아니라 환경변수에 두면 무엇을 얻고 무엇을 잃는가? 얻는 쪽을 "공격면" 관점에서 설명하라.

**1-I-4.** "온프레미스 안에서 신청한 사람은 다 직원"이라는 전제가 틀린 이유를 쓰라. 가입 승인 3정책과, 필터가 쓰는 조건 두 부류를 대라.

**1-I-5.** 승인 권한을 한 사람에게 몰면 어떤 일이 생기는가? 이 문제의 일반형을 한 문장으로 말하고 해법 두 가지를 쓰라.

**1-I-6.** 관리 대시보드에는 AI가 없다. 그런데도 이것을 먼저 만드는 이유는? 반드시 먼저 만들어야 하는 것 넷을 대라.

### 묶음 J — 3강: 세션과 감사 · 3문항

**1-J-1.** 세션의 유휴 만료와 기록 보관 기간은 왜 별개 설정인가? 각각의 목적을 쓰라.

**1-J-2.** 세션 헤더 이름과 쿠키 이름을 설정으로 빼야 하는 이유를 쓰라. 그리고 **둘 다** 필요한 이유는 무엇인가?

**1-J-3.** 서버가 죽는 원인 1위는 무엇인가? 그것이 이 시스템의 어떤 설정으로 이어지는가?

### 묶음 K — 3강: 구조와 가드레일 · 5문항

**1-K-1.** 도메인 패키지를 `common`·`server`·`front`로 나누는 기준은 무엇인가? 통신 프로토콜 타입이 어디로 가야 하고, 양쪽에 따로 선언하면 무엇을 잃는가?

**1-K-2.** 퍼미션을 미들웨어 한 층에 두고 기본값을 거부로 하는 이유를 쓰라. 엔드포인트마다 검사를 붙이면 무엇이 위험한가?

**1-K-3.** 스킬에 규칙을 적어 두는 것만으로 부족한 이유는? 무엇을 함께 줘야 규칙이 지켜지는가?

**1-K-4.** 포트를 5자리로 강제하는 이유와, 파일 400줄 상한의 목적을 각각 쓰라. 두 규칙의 공통점은?

**1-K-5.** i18n 리소스 키를 타입으로 선언하는 이유를 쓰라. 문자열로 쓰면 오타가 언제 드러나는가?

### 묶음 L — 3강: 모델과 조직 · 5문항

**1-L-1.** 프리셋이란 무엇인가? 왜 등록 단위가 "모델"이 아닌지 예를 들어 설명하라.

**1-L-2.** 코딩용 샘플링 기본값 셋을 쓰라. top_k를 20으로 두면 무엇이 문제이고, repeat_penalty는 언제 주는가?

**1-L-3.** 조직 루트를 하나로 만드는 것이 왜 흔한 실수인가? 복수 허용이 기본값이어야 하는 이유를 "비대칭"으로 설명하라.

**1-L-4.** 조직이 존재하는 큰 이유 하나를 대라. 할당의 기본값은 무엇이며 할당 단위는 무엇인가?

**1-L-5.** 관리자 권한 범위 두 종류를 쓰고, 하위 계층 전체 범위가 없으면 무엇이 안 되는지 설명하라. 위임의 제약은?

### 묶음 M — 3강: 진단과 판단 · 4문항

**1-M-1.** 컨테이너 안의 서비스가 호스트에서 돌는 추론 서버에 연결되지 않는다. 원인은? 이 함정이 특히 잘 걸리는 이유는?

**1-M-2.** 작은 모델이 같은 말을 반복한다. 모델 등록 설정에서 꺼낼 수 있는 처방 두 가지를 쓰라.

**1-M-3.** 엔드포인트를 등록했는데 모델 목록이 자동으로 안 잡힌다. 무엇을 확인하고, 시스템은 어떤 경로를 남겨 둬야 하는가?

**1-M-4.** "AI가 코드를 쓸 때 단위 테스트는 옛날 방식"이라는 주장의 근거를 쓰라. 그리고 이 주장이 **성립하지 않는 영역**을 예와 함께 대라.
...
**2-9.** (3강) 고객사가 "가입은 자동 승인으로 해 주세요, 사내망이라 안전합니다"라고 요구한다. 반대할 근거가 있는가? 받아들일 조건과, 대안 설계를 함께 제시하라.

**2-10.** (3강) 신규 개발자가 "프리셋 대신 클라이언트가 temperature를 보내게 하면 화면도 단순해지고 유연하다"고 제안한다. 무엇이 무너지는지 지적하고, 그 유연성이 정말 필요한 경우의 정상 경로를 제시하라.

**2-11.** (3강) 아래 화면은 계정 정책 설정이다. 이 화면에서 **성격이 다른 두 시간 설정**을 찾아 각각의 목적을 쓰고, 둘을 하나로 합치면 무엇을 잃는지 설명하라.

![계정 정책](../docs/ep03-admin-implementation/assets/frames/t00-13-18.jpg)

**2-12.** (3강) 팀이 "조직도는 우리 회사에 하나니까 루트를 하나로 두고 단순하게 가자"고 한다. 반대 근거를 대고, 만약 그대로 갔을 때 나중에 지사가 생기면 어디를 고쳐야 하는지 열거하라.
...
**3-6. 세션 생명주기** — `src/e03-02-01-session-lifecycle/index.ts` (과제 `e03-02-01`)

유휴 만료·강제 제거·보관 기간 정리를 구분해 구현한다. 만료와 삭제를 한 사건으로 합치면 감사 기록이 사라지고, 만료 확인 없이 갱신하면 죽은 세션이 되살아난다.

- 소요 예상: 30~40분
- 막히면: [`../docs/ep03-admin-implementation/02-session-and-audit.md`](../docs/ep03-admin-implementation/02-session-and-audit.md)

**3-7. 가입 승인 필터** — `src/e03-03-01-signup-filter/index.ts` (과제 `e03-03-01`)

CIDR 대역 판정과 승인 정책 3종을 구현한다. 필수 필드 검사 순서, `pending`과 `rejected`의 구분, 조건 결합 방향이 판정 대상이다.

- 소요 예상: 40~50분
- 막히면: [`../docs/ep03-admin-implementation/03-signup-policy.md`](../docs/ep03-admin-implementation/03-signup-policy.md)

**3-8. 모델 프리셋 레지스트리** — `src/e03-05-01-model-presets/index.ts` (과제 `e03-05-01`)

같은 모델을 파라미터만 달리해 여러 개 등록할 수 있어야 한다. 중복 판정의 기준을 잘못 고르면 그것이 막히고, 클라이언트 파라미터를 병합하면 조직별 통제가 무너진다.

- 소요 예상: 30~40분
- 막히면: [`../docs/ep03-admin-implementation/05-model-registry.md`](../docs/ep03-admin-implementation/05-model-registry.md)

**3-9. 조직 트리와 모델 라우팅** — `src/e03-06-01-org-tree/index.ts` (과제 `e03-06-01`)

다중 루트, 순환 방어, 관리자 권한 범위 2종, 가장 가까운 조상의 모델 할당을 구현한다.

- 소요 예상: 50~70분
- 막히면: [`../docs/ep03-admin-implementation/06-org-tree-and-routing.md`](../docs/ep03-admin-implementation/06-org-tree-and-routing.md)
...
- **"사내망이면 안전하다"로 정리했는가?** → 3강 03장. 사내망은 네트워크 위치일 뿐 신원 증명이 아니다.
- **프리셋을 "모델 목록"으로 이해했는가?** → 3강 05장. 등록 단위는 모델 + 파라미터 조합이고, 그래서 이름이 키다.
- **조직도의 루트가 하나라고 전제했는가?** → 3강 06장 + 2-12. 되돌리기가 비싼 쪽으로 기울지 말 것.
- **"단위 테스트는 이제 필요 없다"로 정리했는가?** → 3강 08장 + 1-M-4. 전제가 붙은 주장이다 — 순수 로직에는 여전히 명세로 쓴다.
