...
두 유예 기간은 **같은 순간에 시작해 서로 다른 대상을 재는 두 시계**다. 여기에 헬스체크가 30초에 한 번만 온다는 사실이 겹친다.

![태스크가 RUNNING이 된 뒤 ALB 쪽 유예 60초와 컨테이너 쪽 StartPeriod 30초가 동시에 흐르고, 30초 간격 프로브가 연속 2회 성공해야 healthy가 되며, 그 뒤 옛 태스크가 등록 해제 지연 30초 동안 draining으로 남는다](assets/diagrams/deploy-clocks.png)

**유예 기간이 끝나는 시점(60초)이 ALB가 healthy를 확정할 수 있는 가장 빠른 시점과 같다** — 그래서 애플리케이션 준비가 60초를 넘기면 그때부터의 실패는 카운트에 들어가고, 태스크가 교체되기 시작한다. (등록 해제 지연의 **기본값 300초**는 이 그림의 시간축에 넣으면 나머지 전부를 눌러 버려서 뺐다. 값의 의미는 [03](03-alb-and-target-group.md)에 있다.)
