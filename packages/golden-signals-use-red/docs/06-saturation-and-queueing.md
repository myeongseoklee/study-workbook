...
![사용률에 따른 지연 배수 곡선 — 70%까지 평평하다가 90% 뒤로 벽처럼 선다](assets/diagrams/latency-multiplier.png)

**곡선의 모양이 이 절의 주장 전부다.** 70%까지는 거의 평평해서 "여유가 있다"는 직관이 맞는 것처럼 보이고, 그 직관을 그대로 들고 90%로 올라가면 벽을 만난다. 100%는 값이 큰 것이 아니라 **정의되지 않는다** — 도착이 처리를 따라잡아 큐가 발산하는 지점이다.
...
![직렬 장치와 병렬 장치의 동시 실행 깊이 — 같은 %util 100%, 다른 여유](assets/diagrams/util-serial-vs-parallel.png)

y축이 **동시에 처리 중인 요청 수**다. 두 선 모두 한 번도 0으로 내려가지 않으므로 `%util`은 똑같이 100%다 — 그 지표가 재는 것이 정확히 "깊이가 0이 아닌 시간의 비율"이기 때문이다. 그런데 한쪽은 깊이 1에 붙어 있고 다른 쪽은 3~4를 오가며 장치가 감당하는 32까지 남아 있다. **같은 100%가 한쪽에선 한계이고 다른 쪽에선 여유다.**
