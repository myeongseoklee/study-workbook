...
Four Golden Signals의 네 신호·함정·인과 사슬·실무 체크리스트·안티패턴이 이 강의의 본문이고, 본문 캡처와 `[MM:SS]` 표기가 그것을 가리킨다. USE·RED는 이 강의에서 도입부 1분(`[00:30]`~`[01:30]`)과 타이틀 슬라이드에 이름만 등장하므로, 이 자료의 [03](03-use-method.md)·[04](04-red-method.md)는 **위 원저자 1차 출처로 새로 채운 것**이다. 어디까지가 강의이고 어디부터가 보강인지는 [00-overview.md](00-overview.md) § 이 자료의 근거에 표로 있다.
