...
지금 시점의 실무 서버는 대부분 dual-era 전환기다. "내 서버가 Claude Desktop 구버전과 최신 Inspector 양쪽에서 동작해야 한다"면 dual-era 서버 패턴(요청 형태로 분기)이 답이고, 클라이언트를 만든다면 stdio 프로브 → 폴백 로직이 첫 구현 과제다. 코딩 과제 04-01이 정확히 이 판정 로직이다.
