...
코딩 과제 08-01이 MRTR 서버 코어다: `input_required` 응답 생성, requestState 무결성 검증, 재시도 처리. 실무에서는 "예약 확정 전 확인", "부족한 파라미터 묻기"가 전부 이 패턴 위에 선다 — Claude Code 같은 호스트의 승인 다이얼로그가 클라이언트 측 구현이다.
