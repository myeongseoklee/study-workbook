 ...
 *
 * 📍 되짚기: docs/06-rag-when-needed.md / docs/90-must-memorize.md § 판단 경계
 */
...
export function retrieve(query: string, docs: string[] = DOCS, k = 2): string[] {
  ...
  return docs
    .map((d) => ({ d, score: d.split("").filter((ch) => q.includes(ch)).length }))
...
export async function answer(query: string, askFn: Ask, docs: string[] = DOCS): Promise<string> {
  ...
  const context = retrieve(query, docs).join("\n");
