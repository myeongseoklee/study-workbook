 ...
 *
 * 명세: tests/8-2-analyst-agent.test.ts (먼저 읽어라)
 * 막히면 정답: solutions/8-2-analyst-agent.ts
...
export async function handleInvoke(askFn: Ask, query: string): Promise<{ result: string }> {
  ...
  throw new Error("TODO: handleInvoke 구현. 막히면 solutions/8-2-analyst-agent.ts 참고");
}
