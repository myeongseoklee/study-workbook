 ...
 *
 * 명세: tests/8-4-orchestrator.test.ts (먼저 읽어라)
 * 막히면 정답: solutions/8-4-orchestrator.ts
 *
...
export async function discover(fetchFn: FetchFn, urls: string[]): Promise<Record<string, AgentCard>> {
  ...
  throw new Error("TODO: discover 구현. 막히면 solutions/8-4-orchestrator.ts 참고");
}
...
export async function invoke(fetchFn: FetchFn, base: string, query: string): Promise<string> {
  ...
  throw new Error("TODO: invoke 구현. 막히면 solutions/8-4-orchestrator.ts 참고");
}
...
export async function runOrchestration(
  ...
  throw new Error("TODO: 오케스트레이션 흐름 구현. 막히면 solutions/8-4-orchestrator.ts 참고");
}
